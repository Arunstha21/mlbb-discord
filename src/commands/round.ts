...
const command: CommandDefinition = {
	...
	executor: async (msg, args, support) => {
		...
		if (args.length < 2) {
			await msg.reply(`Usage: \`dot!round <id> #channel <round>\` or \`dot!round #channel <round>\``);
			return;
		}
		...
		if (args.length === 3) {
			...
			await msg.reply("⚠️ **Missing Round Number**\n\nUsage: `dot!round <id> #channel <round>` or `dot!round #channel <round>`");
			return;
		}
		...
		if (enrolledPlayers.length === 0) {
			await msg.reply("⚠️ **No Players Enrolled**\n\nNo Discord users have enrolled yet. Players must enroll first using `dot!enroll` so they can be pinged in match threads.");
			return;
		}
		...
		if (schedules.length === 0) {
			await msg.reply("⚠️ **No Schedule Imported**\n\nNo match schedules found. Please import the schedule first using `dot!schedule` command with a CSV file containing match times.");
			return;
		}
				...
				for (const m of roundMatches) {
					...
					try {
						const thread = await (targetChannel as TextChannel).threads.create({
							...
							reason: `Round ${roundNumber} match generated by dot!round`
						});
