...
export class PersistentTimer {
	...
	public static async create(
		...
		if (end <= new Date()) {
			throw new Error("Timer end time must be in the future");
		}
			...
			await getConnection().transaction(async entityManager => {
				for (const entity of entities) {
					if (entity.end.getTime() <= nowMilli) {
						// Send final message for expired timers before removing from database
						try {
							await discord.sendMessage(entity.channelId, entity.finalMessage);
						} catch (error) {
							logger.warn(`Failed to send final message for expired timer ${entity.id}:`, error);
						}
