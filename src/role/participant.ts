...
export class ParticipantRoleProvider {
	...
	private async create(server: Guild, tournamentId: string): Promise<Role> {
		const role = await server.roles.create({
			name: `${tournamentId}-player`,
			color: this.color,
			reason: "Auto-created by Dot."
		});
	...
	protected async lazyGet(tournament: Tournament): Promise<ParticipantRole> {
		...
		const role =
			server.roles.cache.find(r => r.name === `${tournament.id}-player`) ||
	...
	public async grant(userId: string, tournament: Tournament): Promise<void> {
		...
		try {
			...
			await member.roles.add(id, "Granted by Dot.");
		} catch (error) {
			logger.warn(`Failed to grant role to user ${userId}: user not in server ${server.name} (${server.id})`);
			throw error;
		}
	}
	...
	public async ungrant(userId: string, tournament: Tournament): Promise<void> {
		...
		try {
			const member = await server.members.fetch(userId);
			await member.roles.remove(id, "Revoked by Dot.");
		} catch (error) {
			logger.warn(`Failed to revoke role from user ${userId}: user not in server ${server.name} (${server.id})`);
			throw error;
		}
	}
		...
		if (tournament.id in this.roleCache) {
			...
			try {
				...
				if (role) {
					...
					logger.warn(`Role ${id} not found in server ${server.name} (${server.id}), skipping deletion`);
				}
			} catch (e) {
				logger.error(e);
			}
			...
			const role = server.roles.cache.find(r => r.name === `${tournament.id}-player`);
			if (role) {
				try {
					await role.delete();
				} catch (e) {
